...
brew install ctx-statusline ai-usage-monitor swiftbar-ai-usage glance
```
...
| `glance` | 原生 macOS 選單列主機狀態工具，含 `glance-cli` 一次性終端快照 |
...
glance                   # 首次執行把 Glance.app 裝到 ~/Applications 並啟動
glance-cli               # 終端輸出一次性主機狀態
# swiftbar-ai-usage：依 brew caveats 把外掛 symlink 進 SwiftBar plugins 目錄
```

formula 皆從源碼建置（無需簽章/公證）。formula 由上游 release 流程發布。
All formulae build from source (no signing/notarization); published through each upstream release flow.
